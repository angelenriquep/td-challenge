...
import { CustomerCreditDomainEvent } from '../../../../src/context/shop/customer/domain/CustomerCreditDomainEvent';
...
describe('CustomerCreditCreated', () => {
  it('should update customer credit and publish event', async () => {
    ...
    const publishedEvent = eventBus.events[0];
    expect(publishedEvent).to.be.instanceOf(CustomerCreditDomainEvent);
    expect(publishedEvent.aggregateId).to.equal(customer.id);
  ...
  //   // Run the customer credit created logic and expect an error
  //   try {
  //     await customerCreditCreated.run(id, credit);
  //     // The code should not reach this point, so fail the test if it does
  ...
  //   // Assert that no event is published
